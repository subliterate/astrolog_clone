...
int handle_data_command(int argc, char *argv[]) {
    ...
    char date_str[MAX_VALUE_LEN] = "4/8/1972";
...
int handle_chart_command(int argc, char *argv[]) {
    ...
    // Default chart data - we'll try to override these with args or config
    char date_str[MAX_VALUE_LEN]; strcpy(date_str, "4/8/1972");
...
void print_predict_usage() {
    ...
    printf("  --start <YYYY-MM-DD>  Start date for the search. (Required)\n");
    printf("  --end <YYYY-MM-DD>    End date for the search. (Required)\n");
    printf("  --body <name>         Transiting body (e.g., Sun, Jupiter). (Required)\n");
    printf("  --target-body <name>  Natal body to transit (e.g., Moon, Mars). (Required)\n");
    printf("  --aspect <name>       Aspect to search for (e.g., Conjunction, Square). (Required)\n");
}
...
int handle_predict_command(int argc, char *argv[]) {
    ...
    if (strcmp(subcommand, "transits") == 0) {
        ...
        if (!opt_start || !opt_end || !opt_body || !opt_target_body || !opt_aspect) {
            fprintf(stderr, "Error: Missing required options for 'predict transits'.\n");
            print_predict_usage();
            return 1;
        }

        Date start_date, end_date;
        if (parse_date_string(opt_start, &start_date) != 0) {
        ...
        char natal_tz_str[MAX_VALUE_LEN] = "0";
        char natal_loc_str[MAX_VALUE_LEN] = "0E00 51N30";
        ...
        printf("Searching for transits from %s to %s for %s transiting %s in %s aspect...\n\n",
            opt_start, opt_end, opt_body, opt_target_body, opt_aspect);
        ...
        final_transit_date_str);
            ...
            if (run_astrolog_command(astrolog_args, raw_output_buffer, OUTPUT_BUFFER_SIZE) == 0) {
                ...
                for (int i = 0; i < aspect_count; i++) {
                    // Filter: Check if body1 matches opt_body, body2 matches opt_target_body, and aspect matches opt_aspect
                    // Need to be careful with exact string matches due to astrolog's shortened names (Sun vs. Sun, Merc vs. Mercu)
                    // For now, simple strcmp. Might need a mapping or fuzzy match later.
                    if (strcmp(current_aspects[i].body1, opt_body) == 0 &&
                        strcmp(current_aspects[i].body2, opt_target_body) == 0 &&
                        strcmp(current_aspects[i].aspect, opt_aspect) == 0) {
